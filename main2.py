...
def get_keyboard(rang, second_rang):
    ...
    if rang == 9:
        markup.row(emojize(':heavy_plus_sign::notebook: Добавить абитуриентов'),
                   emojize(':heavy_plus_sign::money_with_wings: Добавить инвесторов'))
        markup.add('Начать голосование за утверждение бюджета')
    if rang == 8:
        markup.row('Купить токены у пользователей', 'Продать токены пользователям')
    return markup
...
@bot.callback_query_handler(func=lambda call: True and call.data.startswith('projects'))
def project_switch(call):
    ...
    if call.data[-1]=='1':
        ...
        SetState("@" + call.from_user.username, 21, cursor, db)
    elif call.data[-1]=='2':
        projects=GetProjects("@" + call.from_user.username,cursor)
        keyboard=telebot.types.InlineKeyboardMarkup()
        for i in range(0,len(projects)):
            keyboard.add(telebot.types.InlineKeyboardButton(text=projects[i][0],callback_data='edit_project_'+str(projects[i][1])))
        bot.send_message(call.message.chat.id,'Доступные для редактирования проекты',reply_markup=keyboard)
    elif call.data[-1]=='3':
        ...
        bot.send_message(call.message.chat.id, 'Все проекты "Корпуса"', reply_markup=keyboard)
    elif call.data[-1]=='4':
        bot.delete_message(chat_id=call.message.chat.id, message_id=call.message.message_id)

        bot.send_message(call.message.chat.id,'Работа с проектами завершена',reply_markup=get_keyboard(GetRang('@' + call.from_user.username, cursor),
                                                    GetSecondRang('@' + call.from_user.username, cursor)))
...
def look_project(call):
    ...
    exit = telebot.types.InlineKeyboardButton(text="Выход", callback_data='projects_4')
    flag = True
    if (GetRang('@' + call.from_user.username, cursor) in [9, 10] or GetSecondRang('@' + call.from_user.username,cursor) in [9, 10]):
        keyboard.add(new_project)
        keyboard.add(edit_project)
        flag = False
...
def editing(call):
    ...
    num=call.data[16]
    if num=='1':
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id, text='Введите никнеймы курсантов, каждый отдельным сообщением. Для завершения введите /stop')
        ...
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                              text='Вкладка <b>"Проекты"</b>', reply_markup=keyboard,parse_mode='HTML')
...
def editing11(call):
    if(call.data[-1]=='%'):
        ...
        keyboard.add(list_project)
        keyboard.add(exit)
        bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text='Вкладка <b>"Проекты"</b>', reply_markup=keyboard,parse_mode='HTML')
    else:
...
def new_project3(call):
    ...
    bot.send_message(call.message.chat.id, 'Так как вы инициировали проект, вы являетесь его главным экспертом.Если вы хотите добавить еще экспертов, введите их никнеймы, каждый отдельным сообщением. Для завершения введите /stop')
...
def new_project4(message):
    ...
    if (nick == '/stop'):
        ...
        exit = telebot.types.InlineKeyboardButton(text="Выход", callback_data='projects_4')
        flag = True
        if (GetRang('@' + message.from_user.username, cursor) in [9, 10] or GetSecondRang('@' + message.from_user.username,
                                                                                       cursor) in [9, 10]):
            keyboard.add(new_project)
            keyboard.add(edit_project)
            flag = False
...
def send_offer(message):
    if(GetRang('@'+message.from_user.username,cursor)==9 or GetSecondRang('@'+message.from_user.username,cursor)==9):
        if(IsThereActiveVoting(cursor)):
            ...
            if(len(words)<3):
                bot.send_message(message.chat.id,'Ошибка в написании команды. Формат: /budget <url> <summ>',
                                 reply_markup=get_keyboard(GetRang('@' + message.from_user.username, cursor),
                                                           GetSecondRang('@' + message.from_user.username, cursor)))
            else:
                url=words[1]
                ...
                bot.send_message(message.chat.id,'Утверждается бюджет на следующий месяц\nПодробности по ссылке: '+url+'\nИтоговая сумма: '+budget)
                members=GetListOfUsers(cursor)
                keyboard = telebot.types.InlineKeyboardMarkup()
                keyboard.row(telebot.types.InlineKeyboardButton(text="За", callback_data="voting_1"))
                keyboard.row(telebot.types.InlineKeyboardButton(text="Против", callback_data="voting_2"))
                for i in range(0,len(members)):
                    if(members[i][3]!=None):
                       bot.send_message(members[i][3],'Началось голосование за утверждение бюджета. Подробности по ссылке: '+url+'\nИтоговая сумма: '+budget+
...
def voting(call):
    ...
    bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                          text='Спасибо за ваш голос! Текущий прогресс голосования:\n'+BudgetInfo(cursor),
                          reply_markup=get_keyboard(GetRang('@' + call.from_user.username, cursor),
                                                    GetSecondRang('@' + call.from_user.username, cursor)))
...
def set_rang(message):
    if(message.chat.type=='private' and (GetRang('@'+message.from_user.username,cursor)==9 or GetSecondRang('@'+message.from_user.username,cursor)==9)):
        ...
        if(len(words)>1):
            ...
            if(IsUserInDB(nick,cursor,db)):
                keyboard=telebot.types.InlineKeyboardMarkup()
                keyboard.add(telebot.types.InlineKeyboardButton(text='Первый курс',callback_data='set_rang_2#'+nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Второй курс', callback_data='set_rang_3#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Третий курс', callback_data='set_rang_4#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Тьютор', callback_data='set_rang_5#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Преподаватель', callback_data='set_rang_6#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Сталкер', callback_data='set_rang_7#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Инвестор', callback_data='set_rang_8#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Администратор', callback_data='set_rang_9#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Эксперт', callback_data='set_rang_10#' + nick))
                keyboard.add(telebot.types.InlineKeyboardButton(text='Завуч', callback_data='set_rang_11#' + nick))
...
def text(message):
    if message.chat.type == 'private':
        ...
        if mess == '📂 Проекты':
            ...
            add_abit(message)
        elif mess == 'Посмотреть свой профиль':
            get_profile(message)
